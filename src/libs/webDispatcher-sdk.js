    ...
    this.play = function () {
        ...
        playInterval = setInterval(function () {
            ...
            if ((audioData.audioOffset + 1) * decodeLength <= voiceBuffer.length) {
                ...
                //pcm转wav
                var wavData = audioData.encodeWav(splVoice);
                //console.log(wavData);

                //实时播放wav
                audioData.playWav(wavData);
...
function handleVoice(data) {
    ...
    if (isReceive) {
        ...
        //解码后加入缓存区
        var pcmData = sRecorder.deCode(voice);
        for (var i = 0; i < pcmData.length; i++) {
            voiceBuffer.push(pcmData[i]);
        }

        //缓存2帧后播放
        if (!isPlaying && voiceBuffer.length > 320) {
            sRecorder.play();
        }
    }
}
